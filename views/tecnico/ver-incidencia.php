...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'agregar_comentario') {
    try {
        ...
        if (!empty($comentario)) {
            if ($incidencia->agregarComentario($incidencia_id, $_SESSION['user_id'], $comentario, 'COMENTARIO', $publico)) {
...
<div class="row mb-4">
    <div class="col-12">
        ...
        <?php if (in_array($incidencia->id_stat, [2, 3, 4])): // No mostrar si está resuelta o cerrada ?>
                    ...
                    <div class="col-md-6">
                        <table class="table table-borderless">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $fecha_inicio = new DateTime($incidencia->fecha_inicio);
                                    $fecha_actual = new DateTime();
                                    
                                    if ($incidencia->fecha_terminacion) {
                                        $fecha_fin = new DateTime($incidencia->fecha_terminacion);
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($incidencia->fecha_terminacion): ?>
                                        <?php echo date('d/m/Y H:i', strtotime($incidencia->fecha_terminacion)); ?>
                                    <?php else: ?>
