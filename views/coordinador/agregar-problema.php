...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (empty($titulo) || empty($descripcion) || empty($id_categoria) || empty($id_impacto) || empty($id_stat)) {
            ...
            // Crear el problema
            if ($problema->create()) {
                // Redireccionar a la página de problemas con mensaje de éxito
                header("Location: problemas.php?success=created");
                exit;
            } else {
                $error = "Error al crear el problema. Por favor intente nuevamente.";
            }
        }
...
<!-- Formulario para registrar problema -->
<div class="row">
    <div class="col-12">
        <div class="card">
                ...
                <form action="" method="POST">
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="titulo" name="titulo" required maxlength="100" value="<?php echo htmlspecialchars($_POST['titulo'] ?? ''); ?>">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="impacto" name="impacto" required>
                                ...
                                <?php foreach ($impactos as $impacto): ?>
                                    <option value="<?php echo $impacto['ID']; ?>" <?php echo (isset($_POST['impacto']) && $_POST['impacto'] == $impacto['ID']) ? 'selected' : ''; ?>>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="estado" name="estado" required>
                                ...
                                <?php foreach ($estados as $estado): ?>
                                    <option value="<?php echo $estado['ID']; ?>" <?php echo (isset($_POST['estado']) && $_POST['estado'] == $estado['ID']) ? 'selected' : ''; ?> <?php echo ($estado['ID'] == 1) ? 'selected' : ''; ?>>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="responsable" name="responsable">
                                ...
                                <?php foreach ($responsables as $responsable): ?>
                                    <option value="<?php echo $responsable['ID']; ?>" <?php echo (isset($_POST['responsable']) && $_POST['responsable'] == $responsable['ID']) ? 'selected' : ''; ?>>
                                        <?php echo htmlspecialchars($responsable['Nombre']); ?> (<?php echo htmlspecialchars($responsable['Rol']); ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <button type="submit" class="btn btn-success">
                                <i class="fas fa-save me-2"></i>Registrar Problema
                            </button>
